...
//        if ($this->backup_file_ext == 'gz') {
    ...
    private function getCategoryTree()
    {
        ...
        $iterator = new RecursiveIteratorIterator(
            new RecursiveDirectoryIterator($this->shop_cart->getShopRootDir().$dir),
...
//        echo '1|'.$this->shop_cart->jsonEncode($this->searched_files, JSON_FORCE_OBJECT);
        echo $this->shop_cart->jsonEncode(
            array(
                ...
                $this->shop_cart->getKeyMessage()   => $this->searched_files,
            )
        );
    }
        ...
        if ($fp = opendir($path)) {
            while ($value = readdir($fp)) {
                if (is_file($path.'/'.$value)) {
                    ...
                    $this->getFiles($path.'/'.$value, $mask, $ignore_dir, $include_subdir);
                }
            }
...
//        }
    ...
    private function getStoreFileArchive()
    {
        ...
        if (!is_writable($this->tmp_folder_path)) {
            $this->generateError($this->br_errors['not_writeable_dir']);
        }
