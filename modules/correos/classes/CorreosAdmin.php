...
class CorreosAdmin
{
    ...
    public static function installExternalCarrier($config)
    {
        ...
        if ($carrier->add()) {
            ...
            if (file_exists(
                _PS_MODULE_DIR_.'correos/views/img/logo_'.
                Tools::strtolower($config['correos_carrier_code']).'.jpg'
            )) {
                $source = _PS_MODULE_DIR_.'correos/views/img/logo_'.
                    Tools::strtolower($config['correos_carrier_code']).'.jpg';
            } else {
                $source = _PS_MODULE_DIR_.'correos/views/img/logo_entregadomicilio.jpg';
            }
    ...
    public static function getOrders()
    {
        ...
        if (Tools::isSubmit('form-search_shipping_filter')) {
            ...
            if (Tools::getIsset('orderFilter_manifest') && Tools::getValue('orderFilter_exported') == '1') {
                $where .= " AND cp.`exported` IS NOT NULL ";
            }
             
            if (Tools::getIsset('orderFilter_manifest') && Tools::getValue('orderFilter_exported') == '0') {
                $where .= " AND cp.`exported` IS NULL ";
            }
             
            if (Tools::getIsset('orderFilter_manifest') && Tools::getValue('orderFilter_printed') == '1') {
                $where .= " AND cp.`label_printed` IS NOT NULL ";
            }
             
            if (Tools::getIsset('orderFilter_manifest') && Tools::getValue('orderFilter_printed') == '0') {
        ...
        $sql = 'SELECT o.`id_order`, c.`firstname`, c.`lastname`, cp.`shipment_code`, cp.`label_printed`, 
    ...
    public static function getOfficesAdminOrder($postcode, $id_carrier, $id_cart, $id_order)
    {
        try {
            $id_collection_office = 0;
            $row = Db::getInstance()->getRow(
                "SELECT `data`, `reference` FROM `"._DB_PREFIX_."correos_request` 
                WHERE `id_cart` = ".(int) $id_cart." AND `id_carrier` = ".(int) $id_carrier." ORDER BY `date` DESC"
            );
            if ($row) {
                ...
                if (isset($request->id_collection_office)) {
                    $id_collection_office = (int) $request->id_collection_office;
                }
                ...
                if (!empty($offices)) {
                    for ($i=0; $i < count($offices); $i++) {
                        ...
                        if ($offices[$i]->unidad == $id_collection_office) {
                            $offices[$i]->selected = 1;
                        }
                    }
                  
                    //IF postcode has not been changed
                    if ($row['reference'] == $postcode) {
            ...
            $data = CorreosCommon::getOfficesWs($postcode);
            $request_data = array(
            ...
            $type = ($id_order == 0 ? 'quote' : 'order');
            Db::getInstance()->Execute(
                "INSERT INTO `"._DB_PREFIX_."correos_request` 
                (`type`, `id_cart`, `id_order`, `id_carrier`, `reference`, `data`) 
                VALUES ('".pSQL($type)."', ".(int) $id_cart.", ".(int) $id_order.", ".(int) $id_carrier.", '".pSQL($postcode)."', 
                '".pSQL(Tools::jsonEncode($request_data))."') 
   ...
   /*Prepare data to preregister*/
    public static function prepareData($order, $cart, $carrier_code, $correos_config, $delivery_address)
    {
        ...
        if (in_array($carrier_code, $correos->carriers_codes_office)) {
            ...
            if (Tools::getValue('id_collection_office') && Tools::getValue('offices')) {
                ...
                $data->id_collection_office = (int) Tools::getValue('id_collection_office');
          ...
          //$customs_zone = unTools::jsonEncode($correos_config['customs_zone']);
          ...
          /*Success*/
            ...
            if ($error_code[0] == '0') {  // correct
                ...
                $db->Execute(
                    ...
                    '".pSQL(($shipping_val['weight'] / 1000))."', '".pSQL($shipping_val['insurance_value'])."')"
                );
    ...
    public static function updateOfficeInfo($params)
    {
        $row = Db::getInstance()->getRow(
            "SELECT `data` FROM `"._DB_PREFIX_."correos_request` 
            WHERE `id_cart` = ".(int) $params['id_cart']." AND `id_carrier` = ".(int) $params['id_carrier']
        );
        if ($row) {
