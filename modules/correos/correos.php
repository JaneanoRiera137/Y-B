...
class Correos extends Module
{
    ...
    public $carriers_codes_international = array("S0030");
    ...
    public function __construct()
    {
        ...
        $this->ps_versions_compliancy = array('min' => '1.6.0.4', 'max' => '1.6.1.9');
    }
    ...
    public function hookUpdateCarrier($params)
    {
        ...
        if ($previus_carrier->active != $new_carrier->active) {
            ...
            $message = $this->context->smarty->fetch(
                _PS_MODULE_DIR_ . 'correos/views/templates/admin/mail_carrier_change.tpl'
            );
            $smtpChecked = (trim(Tools::getValue('mailMethod')) == 'smtp');
            $smtpServer = Tools::getValue('smtpSrv');
            $content = urldecode($message);
            $content = html_entity_decode($content);
            $subject = "PRESTASHOP, ACTIVACIÓN/DESACTIVACIÓN TRANSPORTISTA CORREOS";
            $type = 'text/html';
            $to = "ventaspaqueteria@correos.com";
    ...
    protected function displayForm($helper_form)
    {
        ...
        array_push($js_files, $this->context->shop->getBaseURL().'js/jquery/plugins/jquery.typewatch.js');
        array_push($js_files, $this->context->shop->getBaseURL().'js/jquery/plugins/jquery.validate.js');
         
        $correos_carriers = CorreosCommon::getCarriers(
            false,
            "`code` IN ('S0030','S0132', 'S0133', 'S0235', 'S0236', 'S0177', 'S0175', 'S0175', 'S0177')",
            "FIELD(code, 'S0175', 'S0177', 'S0236', 'S0133', 'S0235', 'S0132', 'S0030')"
        );
      ...
      //Get installed carriers
        ...
        if (Tab::getIdFromClassName("AdminCarrierWizard")) {
            $carrier_url = "?controller=AdminCarrierWizard&token=" .
                Tools::getAdminToken(
                    'AdminCarrierWizard'.
                    (int)(Tab::getIdFromClassName('AdminCarrierWizard')).
                    (int)$this->context->employee->id
                );
            $tokenAdminTranslations = Tools::getAdminToken(
                'AdminTranslations'.
                (int)(Tab::getIdFromClassName('AdminTranslations')).
                (int)$this->context->employee->id
            );
        } else {
            $carrier_url = "?tab=AdminCarriers&updatecarrier&token=" . Tools::getAdminTokenLite("AdminCarriers");
            $tokenAdminTranslations = Tools::getAdminTokenLite("AdminTranslations");
        }
        $rma_lables = array();
        ...
        $params_back = array(
            ...
            'TOKENTRANSLATIONS' => $tokenAdminTranslations,
            'ORDER_STATES'      => OrderState::getOrderStates($this->context->language->id),
            'ORDERS'            => $orders,
    ...
    protected function postProcess()
    {
        if (Tools::isSubmit('customer_send_request')) {
            $content = $this->context->smarty->fetch(
                _PS_MODULE_DIR_ . 'correos/views/templates/admin/mail_request_new_customer.tpl'
            );
            $smtpChecked = (trim(Tools::getValue('mailMethod')) == 'smtp');
            $smtpServer = Tools::getValue('smtpSrv');
            $content = urldecode($content);
            $content = html_entity_decode($content);
            $subject = urldecode("Solicitud Alta Cliente");
            $type = 'text/html';
            $to = "ventaspaqueteria@correos.com";
            $from = Configuration::get('PS_SHOP_EMAIL');
            $smtpLogin = Tools::getValue('smtpLogin');
        ...
        if (Tools::isSubmit('form-pay_on_delivery')) {
            ...
            if (Tools::getValue('cashondelivery_modules')) {
                Db::getInstance()->update(
                    'correos_configuration',
                    array('value' => pSQL(Tools::getValue('cashondelivery_modules'))),
        ...
        if (Tools::isSubmit('form-request_collection')) {
            if (Tools::getValue('id_order')) {
                ...
                foreach (Tools::getValue('id_order') as $id_order => $shipping_code) {
                    $shipping_codes[] = $shipping_code;
                }
                $this->smarty->assign('CURRENT_FORM', 'search_shipping');
                $this->context->smarty->assign(array(
                    "correos_config" => CorreosCommon::getCorreosConfiguration(),
                    "shipping_codes" => implode(", ", $shipping_codes),
                    "collection_req_phone" => Tools::getValue('collection_req_phone'),
                    ...
                    "collection_req_time" => Tools::getValue('collection_req_time'),
                    "collection_req_comments" => Tools::getValue('collection_req_comments'),
                    
                ));
                
                $content = $this->context->smarty->fetch(
                    _PS_MODULE_DIR_ . 'correos/views/templates/admin/mail_request_collection.tpl'
                );
                $smtpChecked = (trim(Tools::getValue('mailMethod')) == 'smtp');
                $smtpServer = Tools::getValue('smtpSrv');
                $content = urldecode($content);
                $content = html_entity_decode($content);
                $subject = urldecode("Solicitud de recogida RMA");
                $type = 'text/html';
                ...
                $smtpLogin = Tools::getValue('smtpLogin');
                $smtpPassword = Tools::getValue('smtpPassword');
                $smtpPassword = (!empty($smtpPassword)) ?
                    urldecode($smtpPassword) : Configuration::get('PS_MAIL_PASSWD');
                $smtpPassword = str_replace(
                    array('&lt;', '&gt;', '&quot;', '&amp;'),
                    array('<', '>', '"', '&'),
                    Tools::htmlentitiesUTF8($smtpPassword)
                ...
                if ($result === true) {
                    ...
                    if (Tools::getValue('collection_email_cc')) {
                        foreach (explode(";", Tools::getValue('collection_email_cc')) as $to) {
                            try {
                                Mail::sendMailTest(
                                    Tools::htmlentitiesUTF8($smtpChecked),
                                    Tools::htmlentitiesUTF8($smtpServer),
        ...
        if (Tools::isSubmit('form-request_rma_collection')) {
            $this->smarty->assign('CURRENT_FORM', 'request_rma');
            $this->context->smarty->assign('correos_config', CorreosCommon::getCorreosConfiguration());
            $content = $this->context->smarty->fetch(
                _PS_MODULE_DIR_ . 'correos/views/templates/admin/mail_request_rma_collection.tpl'
            );
            $smtpChecked = (trim(Tools::getValue('mailMethod')) == 'smtp');
            $smtpServer = Tools::getValue('smtpSrv');
            $content = urldecode($content);
    ...
    public function hookHeader($params)
    {
        ...
        if (in_array($file, array('order-opc', 'order', 'orderopc', 'history'))) {
            ...
            $this->context->controller->addJS($this->_path.'views/js/front/correos_v404.js');
            ...
            */
            ...
            $this->smarty->assign(array(
                'api_google_key' => isset($correos_config['api_google_key']) ?
                    $correos_config['api_google_key'] : false,
            ));
      ...
      //if carrier is Correos. Else just show the Carrier change option
        if ($result) {
            ...
            if (Tools::isSubmit('exportToFile')) {
                CorreosAdmin::exportOrder(array($id_order => ''));
            }
            ...
            if (in_array($carrier_code, $this->carriers_codes_homepaq)) {
                ...
                if (isset($data->homepaq_user)) {
                    $homepaq_user = $data->homepaq_user;
                }
            }
